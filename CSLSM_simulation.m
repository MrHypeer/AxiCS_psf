...
for ii_psf = 1:length(psf_pxl)
    stack_data = zeros(1024,1024,length(sub_samp)+1);
    ...
    for ii_subsamp = 1:length(sub_samp)
        ...
        % calculate PSNR
        [psnr_value, snr_value] = psnr(rec_norm(1:1024,1:1024), imFull_norm);
        RMSE = sqrt(immse(rec_norm(1:1024,1:1024), imFull_norm));
        ...
        imRec_trim = interp2(XX_rec, YY_rec, rec, XX_ori, YY_ori);
        stack_data(:,:,ii_subsamp+1) = imRec_trim;
    end
    stack_data(:,:,1) = imFull;
